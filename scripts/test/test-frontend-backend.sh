...
if netstat -an | grep -q ":8080.*LISTEN"; then
    ...
    echo ""
    echo "   测试绩效计算接口..."
    curl -s -X GET http://localhost:8080/api/performance/calculation/history \
         -w "\n状态码: %{http_code}\n" || echo "   ✗ 绩效计算接口测试失败"
    
    echo ""
    echo "   测试多维度统计接口..."
    curl -s -X GET http://localhost:8080/api/statistics/multi-dimension/data \
         -w "\n状态码: %{http_code}\n" || echo "   ✗ 多维度统计接口测试失败"
    
    echo ""
    echo "   测试运营评分接口..."
    curl -s -X GET http://localhost:8080/api/statistics/operation-score/data \
         -w "\n状态码: %{http_code}\n" || echo "   ✗ 运营评分接口测试失败"
    
    echo ""
    echo "   测试绩效指标接口..."
    curl -s -X GET http://localhost:8080/api/performance/indicator/list \
         -w "\n状态码: %{http_code}\n" || echo "   ✗ 绩效指标接口测试失败"
...
if netstat -an | grep -q ":3000.*LISTEN"; then
    ...
    # 测试前端页面
    echo "   测试前端首页..."
    curl -s -o /dev/null -w "状态码: %{http_code}\n" http://localhost:3000/ || echo "   ✗ 前端首页访问失败"
